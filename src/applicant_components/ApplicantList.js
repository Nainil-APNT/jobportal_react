...
const ApplicantList = () => {
  ...
  const deleteApplicant = (e,id) => {
          e.preventDefault();
          applicantServiceInstance.deleteApplicant(id).then((res)=> {
                if(applicants){
                  setApplicants((prevElemenet)=>{
                      return prevElemenet.filter((applicant)=>applicant.id !== id)
                    })
                }
          });
  }
            ...
            {!loading && (
              <tbody className='bg-white'>
                {applicants.map((applicant)=>(
                  <Applicant applicant={applicant}  deleteApplicant={deleteApplicant} key={applicant.id}></Applicant>
             ))}
