...
const JobInformationList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Format date strings using date-fns
        const jobInformationsWithFormattedDates = response.data.map((jobInformation) => ({
          ...jobInformation,
          positionDate: format(new Date(jobInformation.positionDate * 1000), 'yyyy-MM-dd HH:mm:ss'),
          applicationDate: format(new Date(jobInformation.applicationDate * 1000), 'yyyy-MM-dd HH:mm:ss'),
        }));
        setJobInformations(jobInformationsWithFormattedDates);
      } catch (error) {
        console.log(error);
      }
      setLoading(false);
  ...
  const deleteJobInformation = (e,jobId) => {
          e.preventDefault();
          jobInformationServiceInstance.deleteJobInformation(jobId).then((res)=> {
                if(jobInformations){
                  setJobInformations((prevElemenet)=>{
                      return prevElemenet.filter((jobInformation)=>jobInformation.jobId !== jobId)
                    })
                }
          });
  }
  ...
  const handleAddJobInformationClick = () => {
    const user = AuthService.getCurrentUser();

    if (user && user.roles.includes("ROLE_ADMIN")) {
      navigate('/addJobInformation');
    } else {
      navigate('/forbidden');
    }
  };
  ...
  return (
    ...
    <div className='container mx-auto my-8'>
      ...
      {/*   <button onClick={()=>navigate("/addJobInformation")} className='rounded bg-slate-600 text-white px-6 py-2'>Add JobInformation</button> */}
        <button onClick={handleAddJobInformationClick}>Add JobInformation</button>
        </div>
          ...
          <table className='min-w-full'>
            <thead className='bg-gray-200'>
                   <tr>
                      ...
                      <th className='text-left font-medium text-gray-800 uppercase tracking-wider py-3 px-6'>Job Location</th>
            ...
            {!loading && (
              <tbody className='bg-white'>
                {jobInformations.map((jobInformation)=>(
                  <JobInformation jobInformation={jobInformation}  deleteJobInformation={deleteJobInformation} key={jobInformation.jobId}>
