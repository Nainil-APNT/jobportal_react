...
const JobInformation = ({jobInformation,deleteJobInformation}) => {
    ...
    const editJobInformation = (e,id)=>{
      e.preventDefault();
      navigate(`/editJobInformation/${id}`);
    }
...
<button
  onClick={(e) => editJobInformation(e, jobInformation.id)}
  className="text-indigo-600 hover:text-indigo-800 px-4 hover:cursor-pointer"
>
  Edit
</button>
<button
  onClick={(e) => deleteJobInformation(e, jobInformation.id)}
