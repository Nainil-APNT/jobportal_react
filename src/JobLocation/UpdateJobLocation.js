...
const UpdateJobLocation = () => {
  ...
  useEffect(()=>{
      const fetchData = async() => {
        try {
            const response =await jobLocationServiceInstance.getJobLocationById(id);
  ...
  const UpdateJobLocation =(e)=>{
      e.preventDefault();
      jobLocationServiceInstance.UpdateJobLocation(jobLocation,id)
      .then((response)=> {
    ...
    <div className="px-8 py-8">
      ...
      <div className="font-bold text-2xl tracking-wider">
        ...
        <div className="items-center justify-center h-14 w-full my-4 space-x-4 pt-4">
          <button onClick={UpdateJobLocation} className='rounded text-white font-semibold bg-blue-600 hover:bg-blue-800 py-2 px-6'>Update</button>

          <button onClick={()=>navigate("/jobLocationList")} className='rounded text-white font-semibold bg-red-500 hover:bg-red-800 py-2 px-6'>Cancel</button>
