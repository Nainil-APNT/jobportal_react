...
const JobLocationList = () => {
  ...
  const deleteJobLocation = (e,id) => {
          e.preventDefault();
          jobLocationServiceInstance.deleteJobLocation(id).then((res)=> {
                if(joblocations){
                  setJobLocations((prevElemenet)=>{
                      return prevElemenet.filter((jobLocation)=>jobLocation.id !== id)
                    })
                }
          });
  }
            ...
            {!loading && (
              <tbody className='bg-white'>
                {joblocations.map((jobLocation)=>(
                  <JobLocation jobLocation={jobLocation}  deleteJobLocation={deleteJobLocation} key={jobLocation.id}></JobLocation>
             ))}
